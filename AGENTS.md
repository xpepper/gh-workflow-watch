...
## Project Overview
This is a GitHub CLI extension (written in Bash) that watches GitHub Actions workflow runs by name. The entire extension is a single executable script `gh-workflow-watch` (no file extension) that acts as a wrapper around `gh run list` and `gh run watch`.
